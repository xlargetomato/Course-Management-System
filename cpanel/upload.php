...
// if ($result->num_rows == 0) {
    ...
    if (move_uploaded_file($_FILES["lectureFile"]["tmp_name"], $targetFile)) {
        ...
        $insertQuery = $MM->prepare("INSERT INTO content (pdfurl, subcode, note) VALUES (?, ?, ?)");
        $insertQuery->bind_param("sss", $pdfUrl, $doctorName, $notes);
 
        if ($insertQuery->execute()) {
            $res = mysqli_query($MM,"SELECT * FROM `subjects` WHERE `code` = '$doctorName'");
            $res = $res->fetch_assoc();
            $subj = $res["name"];
            ...
            $action = "uploaded a lecture in course $subj";
